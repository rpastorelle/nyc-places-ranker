...
const DEFAULT_K = 24;
const LS_KEY = "nyc_places_ranker_v6";
...
function generateId(place) {
  // include borough to reduce collisions across boroughs
...
export default function App() {
    ...
    async function load() {
      ...
      const normalized = (Array.isArray(raw) ? raw : [])
        ...
        .map((p) => {
          ...
          return { ...place, id: generateId(place) };
        });

      setPlaces(normalized);

      // Restore only if dataset matches
      const saved = safeParseJSON(localStorage.getItem(LS_KEY), null);
      if (saved && saved.datasetHash) {
        const ids = normalized.map((p) => p.id).sort().join("|");
        if (ids === saved.datasetHash) {
  ...
  useEffect(() => {
    ...
    setRatings((prev) => {
      ...
      for (const p of places) {
        if (typeof next[p.id] !== "number") next[p.id] = DEFAULT_RATING;
      }
      return next;
    });
  }, [places]);
  ...
  }, [places, ratings, wins, losses, comparisonsById, pairCounts, history, excludedIds, kFactor, showLeaderboard]);
  ...
  }, [loading, currentPair, activePlaces, comparisonsById, pairCounts]);

  // Leaderboard for active pool
  const leaderboard = useMemo(() => {
    const rows = activePlaces.map((p) => {
  ...
  }, [activePlaces, ratings, wins, losses, comparisonsById]);
  ...
  function recordResult(winnerId, loserId) {
    ...
    setHistory((prev) => [{ ts: new Date().toISOString(), winnerId, loserId }, ...prev].slice(0, 300));
  ...
  function excludeOneAndReplace(removedId) {
    ...
    const newPair =
      currentPair.a.id === kept.id
        ? { a: kept, b: opponent, key: kept.id < opponent.id ? `${kept.id}__${opponent.id}` : `${opponent.id}__${kept.id}` }
        : { a: opponent, b: kept, key: kept.id < opponent.id ? `${kept.id}__${opponent.id}` : `${opponent.id}__${kept.id}` };
  ...
  function resetAll() {
    const ok = confirm("Reset rankings, history, and exclusions? This clears local progress.");
    if (!ok) return;
...
function PlaceCard({ side, place, rating, wins, losses, onChoose, onExclude, hotkey }) {
  if (!place) {
    return (
  ...
  return (
    ...
    >
      ...
      {/* IMPORTANT: card is a DIV, so this BUTTON is valid and clickable */}
      <button
        type="button"
